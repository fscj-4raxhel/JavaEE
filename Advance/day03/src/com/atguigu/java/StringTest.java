 ...
 * String的使用
 * @author Chris Jin
 * @create 2022-03-25 9:09 AM
 */
public class StringTest {

    /*
    1. 常量(字面量)与常量的拼接结果在常量池中.且常量池中不会存在相同内容的常量
    2. 只要其中有一个是变量,结果就在堆中
    3. 如果拼接结果用intern()方法,返回值就在常量池中

     */

    @Test
    public void test4(){
        String s1 = "JavaEEhadpop";
        String s2 = "JavaEE";
        String s3 = s2 + "hadpop";
        System.out.println(s1 == s3);//false

        final String s4 = "JavaEE";//true
        String s5 = s4 + "hadpop";
    ...
    public void test2(){
        ...
        //通过new + 构造起的方式:此时的s3和s4保存的地址值,是数据在堆空间中开辟空间以后对应的地址值
    ...
    4. String:代表不可变的字符序列.简称:不可变性.
        体现: 1. 当对字符串重新赋值时,需要重新指定内存区域赋值,不能使用原有的value进行赋值
              ...
              3. 当调用String的replace()方法修改指定的字符或字符串时,也要重新指定内存区域赋值,不能使用原有的value进行赋值
